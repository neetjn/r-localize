export function Tag (opts : object) {
  ...
  function localization () {
    ...
    let translation = self.opts.t == 'string' ? self.localize.translate(self.opts.t) : self.localize.translate(self.opts.t.i, self.opts.t.l)
    // # if fallbackContent toggled and translation is the provided fallback, try to use existing node content
    translation = this.localize.options.fallbackContent && translation == this.localize.options.fallback ? nodeContent ? nodeContent : translation : translation
